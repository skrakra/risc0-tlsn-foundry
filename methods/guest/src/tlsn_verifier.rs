...
fn main() {
    ...
    // Parse outer JSON
    let start_json = env::cycle_count();
    ...
    let end_json = env::cycle_count();
    env::log(&format!("JSON parsing cycles: {}", end_json - start_json));

    // Hex-decode bincode payload
    let start_hex = env::cycle_count();
    ...
    let end_hex = env::cycle_count();
    env::log(&format!("Hex decoding cycles: {}", end_hex - start_hex));

    // Bincode-deserialize into Presentation
    let start_bincode = env::cycle_count();
    ...
    let end_bincode = env::cycle_count();
    env::log(&format!("Bincode deserialization cycles: {}", end_bincode - start_bincode));
    ...
    let start_verify = env::cycle_count();
    ...
    let end_verify = env::cycle_count();
    env::log(&format!("Presentation verification cycles: {}", end_verify - start_verify));
    ...
    let start_score = env::cycle_count();
    ...
    let end_score = env::cycle_count();
    env::log(&format!("Score extraction cycles: {}", end_score - start_score));

    // Examplary: enforce minimum score threshold 
    let start_threshold = env::cycle_count();
    match output.score {
    ...
    let end_threshold = env::cycle_count();
    env::log(&format!("Score threshold check cycles: {}", end_threshold - start_threshold));

    let start_commit = env::cycle_count();
    env::commit(&output);
    let end_commit = env::cycle_count();
    env::log(&format!("Final commit cycles: {}", end_commit - start_commit));
} 